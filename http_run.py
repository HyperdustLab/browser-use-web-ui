...
# ====== webui.json Path (relative to http_run.py directory) ======
WEBUI_JSON_PATH = os.path.join(BASE_DIR, "src", "webui", "components", "webui.json")

# ====== webui.py Path and Interpreter ======
...
DEFAULT_WEBUI_PORT = int(os.environ.get("WEBUI_PORT", "7788"))  # Same as webui.py default
...
CURRENT_CDP_URL: Optional[str] = None  # CDP URL for Playwright direct connection
_current_lock = threading.Lock()

# ====== Global Playwright Status (reuse browser/page across requests) ======
...
def write_or_update_webui_json(cdp_URP_value: Optional[str]):
    """
    Write/update cdp_URP to WEBUI_JSON_PATH.
    If cdp_URP_value is None, keep the file unchanged; if it's "" (empty), write empty string.
    """
    logger.info("Writing webui.json ...")
    ...
    logger.info(f"webui.json has been written: {data}")
...
def wait_http_ok(url: str, timeout_s: float = 30.0, interval_s: float = 0.5) -> bool:
    ...
    logger.warning(f"[probe] Timeout: {url} still not ready")
    return False
...
def kill_existing_webui(host: str, port: int):
    logger.info("🛑 Trying to stop existing webui.py ...")
...
def _watch_exit(proc: subprocess.Popen, label: str):
    def _waiter():
        code = proc.wait()
        logger.warning(f"{label} exited, returncode={code}")
...
def launch_webui_process(host: str, port: int):
    ...
    if not is_port_free(host, chosen_port):
        logger.warning(f"Port {chosen_port} is occupied, automatically selecting free port")
    ...
    if ok:
        ...
        logger.warning("webui.py health check not successful (may be ready later)")
...
# -----------------------------
# Playwright: Launch/Connect Browser & Submit Tasks
# -----------------------------
async def _open_fresh_browser_and_submit(
    ...
    browser_cdp_url: Optional[str] = None,  # New: Playwright connects to external browser via CDP
):
    """
    - If browser_cdp_url is provided: prioritize connecting to external browser via CDP
    - Otherwise: launch new specified engine (chromium|firefox|webkit); chromium supports channel / executable_path
    - Open WebUI → Click Run Agent → Find **visible** textarea → Fill → Click Submit Task
    - ⚠️ Don't close browser, keep it for Pause/Resume use
    """
    global _playwright, _browser, _context, _page
    global CURRENT_BROWSER_NAME, CURRENT_BROWSER_CHANNEL, CURRENT_EXEC_PATH, CURRENT_CDP_URL

    # If browser already exists, close it first (before starting new task)
    if _browser is not None:
        try:
            logger.info("Closing existing browser (before starting new task)...")
            await _browser.close()
        except Exception as e:
            logger.warning(f"Exception when closing old browser: {e}")
        finally:
    ...
    if browser_cdp_url:
        # ---- 1) CDP direct connection to external browser (chromium only) ----
        ...
        try:
            ...
            raise RuntimeError(f"connect_over_cdp failed: {e}")
        ...
        # ---- 2) Normal local browser launch ----
        browser_name = (browser_name or "chromium").lower().strip()
        if browser_name not in ("chromium", "firefox", "webkit"):
            logger.warning(f"Unknown browser '{browser_name}', switching to chromium")
    ...
    # Click Run Agent tab (failure not fatal, may already be on that page)
    ...
    for sel in selectors:
        try:
            logger.debug(f"Trying to click tab: {sel}")
            await _page.click(sel, timeout=4000)
            break
        except Exception:
            pass

    # === Key fix: Select "visible" textarea ===
    # First wait for any textarea to appear on page (attached), then filter visible elements
    try:
        await _page.wait_for_selector("textarea", timeout=max_wait_ms, state="attached")
    except Exception as e:
        raise RuntimeError(f"Page has no textarea: {e}")

    # Find first visible textarea
    textareas = _page.locator("textarea")
    count = await textareas.count()
    target = None
    for i in range(min(count, 25)):  # Check up to 25 to avoid extreme lag
    ...
    if target is None:
        # Fallback: use first one, force scroll into view then fill
    ...
    try:
        ...
        # Fallback: click focus + type
        try:
            await target.click(timeout=3000)
            await _page.keyboard.type(task_text, delay=20)
        except Exception as e:
            raise RuntimeError(f"Cannot fill task in textarea: {e}")

    # Click Submit Task (prioritize text matching; fallback: first visible button)
    try:
        ...
        for i in range(min(bcount, 30)):
            ...
            try:
                ...
                continue
        if not clicked:
            raise RuntimeError("No clickable button on page (cannot submit task).")

    logger.info("✅ Task submitted (browser kept open for Pause/Resume use)")

# -----------------------------
# Click buttons on current page (Pause/Resume)
# -----------------------------
...
async def _close_browser_if_any():
    ...
    if _browser is not None:
        try:
            ...
            logger.warning(f"Exception when closing browser: {e}")
...
class AsyncWorker:
    ...
    def submit_coro(self, coro: Awaitable):
        if self.loop is None:
            raise RuntimeError("AsyncWorker's event loop is not ready.")
        return asyncio.run_coroutine_threadsafe(coro, self.loop)

# -----------------------------
# FastAPI
# -----------------------------
app = FastAPI(title="UI Task Runner", version="2.5.0")
worker = AsyncWorker()
...
class ActionReq(BaseModel):
    ...
    task: Optional[str] = None   # Required when Start
    ...
    browser_channel: Optional[str] = None         # Only effective for chromium: "chrome" / "msedge"
    executable_path: Optional[str] = None         # Only effective for chromium: browser exe path

    # ✨ New: Direct connection to external browser via CDP (field name you requested: browser_cdp__url)
...
def handle_action(req: ActionReq):
    ...
    if action == "start":
        ...
        async def _job_start():
            async with _task_lock:
                # Close old browser (close before starting new task)
                ...
                # Write json (WebUI internal browser)
                ...
                # Open browser and submit (do not close browser)
                await _open_fresh_browser_and_submit(
                    ...
                    browser_cdp_url=req.browser_cdp__url,  # CDP direct connection
                )
        ...
        try:
            ...
            msg = f"Start accepted, WebUI: {webui_url}. Browser kept open (engine={bn}"
            if bc:
                msg += f", channel={bc}"
            if ep:
                msg += f", exec={ep}"
            ...
            if req.cdp_URP is not None:
                msg += f" Updated {WEBUI_JSON_PATH} cdp_URP."
            return ActionResp(
                status="accepted",
                message=msg,
                webui_url=webui_url,
    ...
    if action in ("pause", "resume"):
        ...
        try:
            ...
            msg = f"{req.Action} accepted (clicked {btn} directly on current page)."
            return ActionResp(
                status="accepted",
                message=msg,
                webui_url=webui_url,
    ...
    if action == "stop":
        ...
        try:
            ...
            fut.result()
            return ActionResp(status="ok", message="Stop completed: browser closed and webui.py terminated.")
        except Exception as e:
...
# -----------------------------
# Entry: Only start API (do not start WebUI)
# -----------------------------

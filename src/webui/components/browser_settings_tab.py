...
# Use environment variable WEBUI_JSON_PATH if set, otherwise use default path
...
def load_cdp_url_from_webui_json() -> Optional[str]:
    ...
    Fallback: environment variable BROWSER_CDP; return None if still not found.
    """
...
def create_browser_settings_tab(webui_manager: WebuiManager):
    ...
    with gr.Group():
        with gr.Row():
            # ✅ Dynamically read webui.json when rendering UI (not during module import)
            cdp_url = gr.Textbox(
                label="CDP URL",
